...
const InviteSchema = z.object({ toUsername: z.string().min(1).optional(), toUserId: z.string().min(1).optional(), groupId: z.string().min(1) })
...
router.get("/users", async (req, res) => {
  try {
    ...
    const users = await db.user.findMany({ where: { id: { not: userId } }, select: { id: true, username: true } })
  ...
  try {
    ...
    const created = await db.group.create({
      data: {
        ...
        members: { create: { user: { connect: { id: ownerId } }, role: "owner" } },
      } as Prisma.GroupCreateInput,
    })

    res.status(201).json({ success: true, group: created })
  } catch (err) {
    if (err instanceof z.ZodError) return res.status(400).json({ success: false, errors: z.flattenError(err) })
...
router.post("/:id/invite", async (req, res) => {
  try {
    ...
    if (payload.toUserId) {
      ...
      toUser = await db.user.findFirst({ where: { username: payload.toUsername } })
    }
...
router.post("/invitations/:id/accept", async (req, res) => {
  try {
    ...
    if (invitation.toId !== userId) return res.status(403).json({ success: false, message: "forbidden" })

    // mark accepted and add to members
    await db.invitation.update({ where: { id }, data: { status: "ACCEPTED" } })
    await db.groupMember.create({ data: { group: { connect: { id: invitation.groupId } }, user: { connect: { id: userId } } } })
...
router.get("/:id/messages", async (req, res) => {
  try {
    ...
    const messages = await db.message.findMany({ where: { groupId: id }, include: { sender: true }, orderBy: { createdAt: "asc" } })
...
router.get("/:id/members", async (req, res) => {
  try {
    ...
    const members = await db.groupMember.findMany({ where: { groupId: id }, include: { user: true } })
    const users = members.map((m) => ({ id: m.user.id, username: m.user.username }))
    res.json({ success: true, users })
  } catch (err) {
...
router.post("/:id/messages", async (req, res) => {
  try {
    ...
    const created = await db.message.create({
      data: { group: { connect: { id } }, sender: { connect: { id: userId } }, content: payload.content } as Prisma.MessageCreateInput,
      include: { sender: true },
    })
...
router.get("/", async (req, res) => {
  try {
    const userId = req.session!.user.id
    const memberships = await db.groupMember.findMany({ where: { userId }, include: { group: true } })
